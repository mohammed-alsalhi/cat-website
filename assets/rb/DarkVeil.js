// Port of React Bits <DarkVeil> (OGL) to a vanilla ES module. Shaders are verbatim from the source.
// Note: DarkVeil has no color prop (it is a fixed CPPN palette); use `hueShift` to steer the hue.
...
export default function mount(el, opts = {}) {
  ...
  const render = () => {
    program.uniforms.uTime.value = ((performance.now() - startAt) / 1000) * speed;
  ...
  const update = () => { raf = requestAnimationFrame(update); render(); };
