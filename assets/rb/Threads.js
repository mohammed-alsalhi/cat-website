// Port of React Bits <Threads> (OGL) to a vanilla ES module. Shaders are verbatim from the source.
...
export default function mount(el, opts = {}) {
  const { color = [1, 1, 1], amplitude = 1, distance = 0, enableMouseInteraction = false } = opts;
  ...
  const program = new Program(gl, {
    ...
    uniforms: {
      ...
      // `color` may be an [r,g,b] array (source) or a hex string like '#FFCD11'.
      uColor: { value: typeof color === 'string' ? new Color(color) : new Color(...color) },
  ...
  // resolution to keep large / high-DPI screens smooth.
