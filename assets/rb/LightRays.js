// Port of React Bits <LightRays> (OGL) to a vanilla ES module. Shaders are verbatim from the source.
...
export default function mount(el, opts = {}) {
  const {
    ...
    raysColor = opts.color ?? DEFAULT_COLOR,
    raysSpeed = 1,
    lightSpread = 1,
    rayLength = 2,
    pulsating = false,
  ...
  // Source listens on window, so we do too.
  ...
  const io = new IntersectionObserver(e => { onScreen = e[0].isIntersecting; onScreen ? start() : stop(); }, { threshold: 0.1 });
  io.observe(el);
